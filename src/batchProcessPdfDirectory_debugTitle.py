...
def extract_title_by_author_proximity(text: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        a = author_pattern_a.match(normalized)
        b = author_pattern_b.match(normalized)
        c = stripped.count(',') >= 3 and author_pattern_b.match(normalized[:60])

        if a or b or c:
            print(f"\n  >> Author line candidate at [{i:02d}]: '{stripped[:100]}'")
            print(f"     pattern_a={bool(a)}, pattern_b={bool(b)}, pattern_c={bool(c)}")
            ...
            while start > 0:
                ...
                if author_pattern_b.match(prev_normalized):
                    print(f"     Continuation crawl stopped at [{start-1}] (pattern_b match): '{prev[:80]}'")
                    break
                if (prev.count(',') >= 2
                        ...
                        and not re.search(r'\b(Vol|pp|doi|https?://|Journal of|full article|author affiliation)\b',
                                          prev, re.IGNORECASE)):
                    print(f"     Continuation crawl: skipping [{start-1}]: '{prev[:80]}'")
                    start -= 1
                else:
                    print(f"     Continuation crawl stopped at [{start-1}]: '{prev[:80]}'")
                    break

            print(f"     Collecting title lines above start={start}")
            title_lines = []
            for j in range(start - 1, max(start - 8, -1), -1):
                candidate = lines[j].strip()
                print(f"     j={j}: '{candidate[:80]}'")
                if not candidate:
                    print(f"     -> BREAK: empty")
                    break
                if SKIP_PATTERN.match(candidate):
                    print(f"     -> BREAK: SKIP_PATTERN")
                    break
                if re.search(r'\b(Vol|pp|doi|https?://|Journal of|full article|author affiliation)\b',
                             candidate, re.IGNORECASE):
                    print(f"     -> BREAK: banner guard")
                    break
                if candidate.endswith(('.', ':', '?', '!')):
                    title_lines.insert(0, candidate)
                    print(f"     -> BREAK: ends with punctuation, added: '{candidate[:80]}'")
                    break
                title_lines.insert(0, candidate)
                print(f"     -> Added: '{candidate[:80]}'")

            print(f"     title_lines = {title_lines}")
            if title_lines:
                result = ' '.join(title_lines)
                print(f"     RETURNING: '{result}'")
                return result

    print("  No author line matched — returning ''")
    return ""
